...
struct Source *
lexical_analysis(FILE *filePtr, unsigned int lncnt)
{
	char *raw_line = NULL;
	...
	for (lnum = 1; (fgets(raw_line, LINE_SIZE, filePtr) != NULL); ++lnum)
	{
		...
		{
			printf("Unexpected behavior: token.c 134 - Closing ...");
			exit(1);
		}
...
_strbldr(unsigned int lnum, char *line)
{
	...
	for (token = strtok(line, dlim); token != NULL; token = strtok(NULL, dlim))
	{
		...
		if (tmptknstrct == NULL)
		{
			printf("Unexpected behavior: token.c 209 - Closing ...");
			exit(1);
		}
